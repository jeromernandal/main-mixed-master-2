...
// METAMASK CONNECTION
const TIMEOUT = 1000;
const COLLECTION_NAME = 'Mixed mfers NFT';
let editions = [7777];
let dots = 1;
...
function updateStatusText(isOwner, checking) {
  ...
  if(checking) {
    if(isOwner) {
      statusText.innerText = `You own ${COLLECTION_NAME}!!  Look how many!${renderDots(dots)}`;
    } else {
    ...
    if(isOwner) {
      ...
      statusText.innerText = `You need some ${COLLECTION_NAME} in your collection!`;
    }
  }
